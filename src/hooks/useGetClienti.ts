...
const fetchClienti = async (
  ...
  const { data } = await axiosInstance.get(`${API_BASE_PREFIX}/${ecommerceId}/customers`, {
    params,
  });
...
export const useGetClienti = (page: number = 1, perPage: number = 25, search: string = '') => {
  const { ecommerceId } = useWorkspace();
  return useQuery<PaginatedResponse<Cliente>, Error>({
    queryKey: ['clienti', page, perPage, search],
    queryFn: () => fetchClienti(ecommerceId, page, perPage, search),
    enabled: !!ecommerceId,
  });
};
